...
class WikiWriter:
    ...
    async def write(self, result: ClassificationResult) -> str:
        ...
        If a file already exists at the target location with the same
        ``hash`` in its frontmatter, the write is skipped (the existing
        ...
        if output_path.exists():
            ...
            if existing_hash == result.metadata.hash:
        ...
        frontmatter = {
            ...
            "tags": result.classification.tags or None,
            "category": result.classification.category or None,
            "source": result.metadata.source_path,
            "hash": result.metadata.hash,
        }
        # Strip None values for a cleaner YAML output
        ...
        if result.classification.summary:
            parts.append(result.classification.summary)
            parts.append("")

        # Structured sections from the LLM
        for section in result.classification.sections:
        ...
        # Related pages (wikilink format)
        if result.classification.related_pages:
            parts.append("## Related Pages")
            parts.append("")
            for page in result.classification.related_pages:
                parts.append(f"- [[{page}]]")
            parts.append("")
